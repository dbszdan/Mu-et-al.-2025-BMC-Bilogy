...
%initialfACyto = [0.3];
initialfEis = [0.05];
%initialtensionSValue3 = [0.3*0.013*0.75];
...
% WT, 1.2M -> 0.2M
[maxEpsValue, plotArray, t] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, true, false);
%pause(1000);

% tcb12D, 1.2M -> 0.2M
[maxEpsValue_pil1D, plotArray_pil1D, t_pil1D] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, 0.0, baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, true, false);

% WT, 1.2M -> 0.3M
[maxEpsValue_tricalbD, plotArray_tricalbD, t_tricalbD] = AreaBalancePassValues_Final_20250325(0.9, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, true, false);

% tcb12D, 1.2M -> 0.3M
[maxEpsValue_exoD, plotArray_exoD, t_exoD] = AreaBalancePassValues_Final_20250325(0.9, intialTimeDelay, 0.0, baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, true, false);


% WT, 1.2M -> 0.4M
[maxEpsValue_extCaD, plotArray_extCaD, t_extCaD] = AreaBalancePassValues_Final_20250325(0.8, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, true, false);

% tcb12D, 1.2M -> 0.4M
[maxEpsValue_tricalbO, plotArray_tricalbO, t_tricalbO] = AreaBalancePassValues_Final_20250325(0.8, intialTimeDelay, 0.0, baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, true, false);


% no endocytosis, LatA
[maxEpsValue_endoD, plotArray_endoD, t_endoD] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialtensionSValue3(1), initialCaOut, true, false, false);
%pause(1000)
...
x_values = t;
y_values = [1:length(indexBurstMutants)];
...
color_values = [plotArray_tricalbO(:,12), plotArray_extCaD(:,12), plotArray_exoD(:,12), plotArray_tricalbD(:,12), plotArray_pil1D(:,12), plotArray(:,12)];
...
for i = 1:2:length(indexThreshold)
    if length(indexThreshold) - i == 0
        if indexThreshold(1) ~= -1
            rectangle('Position', [indexThreshold(i)*0.01 5.5 (length(t)-indexThreshold(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
        end
    else
        rectangle('Position', [indexThreshold(i)*0.01 5.5 (indexThreshold(i+1)-indexThreshold(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
    end
end
for i = 1:2:length(indexThreshold_pil1D)
    if length(indexThreshold_pil1D) - i == 0
        if indexThreshold_pil1D(1) ~= -1
            rectangle('Position', [indexThreshold_pil1D(i)*0.01 4.5 (length(t)-indexThreshold_pil1D(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
        end
    else
        rectangle('Position', [indexThreshold_pil1D(i)*0.01 4.5 (indexThreshold_pil1D(i+1)-indexThreshold_pil1D(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
    end
end
for i = 1:2:length(indexThreshold_tricalbD)
    if length(indexThreshold_tricalbD) - i == 0
        if indexThreshold_tricalbD(1) ~= -1
            rectangle('Position', [indexThreshold_tricalbD(i)*0.01 3.5 (length(t)-indexThreshold_tricalbD(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
        end
    else
        rectangle('Position', [indexThreshold_tricalbD(i)*0.01 3.5 (indexThreshold_tricalbD(i+1)-indexThreshold_tricalbD(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
    end
end
for i = 1:2:length(indexThreshold_exoD)
    if length(indexThreshold_exoD) - i == 0
        if indexThreshold_exoD(1) ~= -1
            rectangle('Position', [indexThreshold_exoD(i)*0.01 2.5 (length(t)-indexThreshold_exoD(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
        end
    else
        rectangle('Position', [indexThreshold_exoD(i)*0.01 2.5 (indexThreshold_exoD(i+1)-indexThreshold_exoD(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
    end
end
for i = 1:2:length(indexThreshold_extCaD)
    if length(indexThreshold_extCaD) - i == 0
        if indexThreshold_extCaD(1) ~= -1
            rectangle('Position', [indexThreshold_extCaD(i)*0.01 1.5 (length(t)-indexThreshold_extCaD(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
        end
    else
        rectangle('Position', [indexThreshold_extCaD(i)*0.01 1.5 (indexThreshold_extCaD(i+1)-indexThreshold_extCaD(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
    end
end
for i = 1:2:length(indexThreshold_tricalbO)
    if length(indexThreshold_tricalbO) - i == 0
        if indexThreshold_tricalbO(1) ~= -1
            rectangle('Position', [indexThreshold_tricalbO(i)*0.01 0.5 (length(t)-indexThreshold_tricalbO(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
        end
    else
        rectangle('Position', [indexThreshold_tricalbO(i)*0.01 0.5 (indexThreshold_tricalbO(i+1)-indexThreshold_tricalbO(i))*0.01 1.0], 'LineStyle', '--', 'LineWidth', 1.5)
    end
end
%{
...
yticklabels(flip(["WT, 0.2M", "{\ittcb1}\Delta{\ittcb2}\Delta, 0.2M", "WT, 0.3M", "{\ittcb1}\Delta{\ittcb2}\Delta, 0.3M", "WT, 0.4M", "{\ittcb1}\Delta{\ittcb2}\Delta, 0.4M"]));
ax = gca;
...
color_values = zeros(6, length(x_values));
...
for i = 1:length(x_values)-1
    ...
    for j = 1:length(Area_values_normalized)
        if Area_values_normalized(j) >= x_values(i) && Area_values_normalized(j) <= x_values(i+1)
            color_values(6,i) = plotArray(j,12) + color_values(6,i);
            count = count + 1;
        end
    end
    if count > 0
    ...
    for j = 1:length(Area_values_normalized_pil1D)
        if Area_values_normalized_pil1D(j) >= x_values(i) && Area_values_normalized_pil1D(j) <= x_values(i+1)
            color_values(5,i) = plotArray_pil1D(j,12) + color_values(5,i);
            count = count + 1;
        end
    end
    if count > 0
    ...
    for j = 1:length(Area_values_normalized_tricalbD)
        if Area_values_normalized_tricalbD(j) >= x_values(i) && Area_values_normalized_tricalbD(j) <= x_values(i+1)
            color_values(4,i) = plotArray_tricalbD(j,12) + color_values(4,i);
            count = count + 1;
        end
    end
    if count > 0
    ...
    for j = 1:length(Area_values_normalized_exoD)
        if Area_values_normalized_exoD(j) >= x_values(i) && Area_values_normalized_exoD(j) <= x_values(i+1)
            color_values(3,i) = plotArray_exoD(j,12) + color_values(3,i);
            count = count + 1;
        end
    end
    if count > 0
    ...
    for j = 1:length(Area_values_normalized_noCa)
        if Area_values_normalized_noCa(j) >= x_values(i) && Area_values_normalized_noCa(j) <= x_values(i+1)
            color_values(2,i) = plotArray_extCaD(j,12) + color_values(2,i);
            count = count + 1;
        end
    end
    if count > 0
    ...
    for j = 1:length(Area_values_normalized_tricalbO)
        if Area_values_normalized_tricalbO(j) >= x_values(i) && Area_values_normalized_tricalbO(j) <= x_values(i+1)
            color_values(1,i) = plotArray_tricalbO(j,12) + color_values(1,i);
            count = count + 1;
        end
    end
    if count > 0
