...
% reference case
[maxEpsValue, plotArray, t] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialStrainSValue3(1), initialCaOut, true, true, false);%pause(1000);

% pil1D
[maxEpsValue_pil1D, plotArray_pil1D, t_pil1D] = AreaBalancePassValues_Final_20250325(0.9, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialStrainSValue3(1), initialCaOut, true, true, false);

% tricalbin delta
[maxEpsValue_tricalbD, plotArray_tricalbD, t_tricalbD] = AreaBalancePassValues_Final_20250325(0.8, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialStrainSValue3(1), initialCaOut, true, true, false);

% no exocytosis, BFA
[maxEpsValue_exoD, plotArray_exoD, t_exoD] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialStrainSValue3(1), initialCaOut, false, true, false);
%pause(1000);

% no external Ca
[maxEpsValue_extCaD, plotArray_extCaD, t_extCaD] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialStrainSValue3(1), 0.0, true, true, false);

% tricalbin overexpression
%2.0x
[maxEpsValue_tricalbO, plotArray_tricalbO, t_tricalbO] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1)*2.0, baseStartingVolume, initialfEis(1), initialStrainSValue3(1), initialCaOut, true, true, false);


% no endocytosis, LatA
[maxEpsValue_endoD, plotArray_endoD, t_endoD] = AreaBalancePassValues_Final_20250325(initialMolarityDelta, intialTimeDelay, initialk5Max(1), baseStartingVolume, initialfEis(1), initialStrainSValue3(1), initialCaOut, true, false, false);
%pause(1000)
...
f2 = scatter(t(1:end), Area_values_normalized_pil1D(1:end), 30, plotArray_pil1D(:,12),"filled");
%f2.MarkerFaceAlpha = 0.0019;
scatter(t(1:end), Area_values_normalized_tricalbD(1:end), 30, plotArray_tricalbD(:,12),"filled")
...
yyaxis right
% figure 1 change here
plot(t, plotArray_endoD(:,20), 'Color', [0.5 0.5 0.5]);
...
if indexThreshold_endoD(1) ~= -1
    for i = 1:1:length(indexThreshold_endoD)
        plot([80-40,150-40], [Area_values_normalized(indexThreshold_endoD(i)), Area_values_normalized(indexThreshold_endoD(i))], 'Color', 'black', 'LineStyle','--', 'LineWidth',1.5)
    end
end
%}
